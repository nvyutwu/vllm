...
class FusedMoE(CustomOp):
    ...
    def _load_w13(
        ...
        # Check dimension compatibility before copy (mixed precision support)
        # Skip loading if dimensions don't match (e.g., BF16 MTP in NVFP4 model)
        if expert_data.shape != loaded_weight.shape:
            import logging
            logger = logging.getLogger(__name__)
            logger.debug(
                f"Skipping expert weight load due to dimension mismatch: "
                f"expected {expert_data.shape}, got {loaded_weight.shape}. "
