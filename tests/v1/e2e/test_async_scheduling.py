        ...
        for override_params in sampling_param_tests:
            ...
            if test_preemption:
                preemptions = _get_count(
                    metrics_before, metrics_after, "num_preemptions"
                )
...
def _get_acceptance_rate(before: list[Metric], after: list[Metric]) -> float:
    draft = _get_count(before, after, "spec_decode_num_draft_tokens")
    accept = _get_count(before, after, "spec_decode_num_accepted_tokens")
